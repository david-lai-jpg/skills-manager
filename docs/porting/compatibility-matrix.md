...
| --- | --- | --- | --- | --- | --- | --- | --- |
| C001 | `scan` | default, `--json`, `--project <path>`, `--include-non-skills` | `0` for successful scan | JSON mode must remain machine-readable; default output is skill-relevant and suppresses obvious non-skill files | Detect inbox, store, Claude/Codex global, project dirs, skill dirs, skill symlinks, broken symlinks; diagnostic mode includes dirs missing `SKILL.md` | No action log | `empty-home`, `inbox-import`, `broken-symlink`, `missing-skill-md` |
...
| C018 | `preset add` | `--mode enable/disable` selects the preset enabled/disabled skill list, `--dry-run`, multiple refs | `0` when all refs resolve; nonzero and atomic on unknown/ambiguous refs | JSON includes before/after entries | Mutates only preset JSON; dry-run writes nothing | Apply logs; dry-run does not log | `preset-crud-apply`, `preset-unknown-id`, `action-log-dry-run` |
